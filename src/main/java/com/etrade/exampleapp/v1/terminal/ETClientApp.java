...
import com.etrade.exampleapp.mystuff.MyLogger;
import com.etrade.exampleapp.mystuff.Portfolio;
import com.etrade.exampleapp.mystuff.outs.DiscordPrinter;
import com.etrade.exampleapp.v1.clients.order.*;
...
public class ETClientApp extends AppCommandLine {
	...
	public static boolean synology = false;
	public static DiscordPrinter printer;
	...
	public void init(boolean flag, boolean synology) {
		ETClientApp.synology = synology;
...
/*	private void initOOauth() {
	...
	public void placeOrder(String symbol, int amount){

		OrderPreview preview_client = ctx.getBean(OrderPreview.class);

		//client.setSessionData(sessionData);

		Map<String,String> inputs = preview_client.getOrderDataMap();

		String accountIdKey = "";

		//The account index we're buying for
		//Assuming this is 1
		String acctKeyIndx = "1";
		try {
			accountIdKey = getAccountIdKeyForIndex(acctKeyIndx);
		...
		/* Fills data to service*/
		preview_client.fillOrderActionMenu(choice,inputs);

		//out.print(" Enter Quantity : ");
		//int qty = KeyIn.getKeyInInteger();
		inputs.put("QUANTITY", String.valueOf(Math.abs(amount)));
		...
		/* Fills data to service*/
		preview_client.fillOrderPriceMenu(choice,inputs);
		...
		//Sets duration to "Good for Day"
		choice = 1;

		preview_client.fillDurationMenu(choice,inputs);

		preview_client.previewOrder(accountIdKey, inputs);

		String previewId = preview_client.previewOrder(accountIdKey, inputs);

		inputs.put("PREVIEW_ID", previewId);

		preview_client.place_da_order(accountIdKey, inputs);
	}

	public void getAccountList(boolean synology) {
		...
		try {
			...
			if (!synology) {
				...
				printer.print(String.format("HttpStatus: %20s", e.getHttpStatus()));
				printer.print(String.format("Message: %23s", e.getMessage()));
				printer.print(String.format("Error Code: %20s", e.getCode()));
			}
		}catch (Exception e) {
	...
	public double get_cash() {
		...
		String accountIdKey = "";
		try {
			accountIdKey = getAccountIdKeyForIndex("1");
		}catch(ApiException e) {
			return 0;
		}
	...
	public Portfolio getPortfolioObject(String acctIndex) {
		...
		try {
			...
			while(acctItr.hasNext()) {
				...
				while (itr.hasNext()) {
					...
					JSONObject prdObj = (JSONObject) innerObj.get("Product");
					responseData[0] = prdObj.get("symbol");
					tickers.add((String) prdObj.get("symbol"));

					responseData[1] = innerObj.get("quantity");
					String temp = Long.toString((Long) responseData[1]);
		...
		return new Portfolio(tickers, nums);
	}
			...
			try {
				...
				if(quoteData != null) {
					...
					while (itr.hasNext()) {
						...
						JSONObject all = (JSONObject) innerObj.get("All");
						if (all != null && all.get("lastTrade") != null) {
							return Double.parseDouble(all.get("lastTrade").toString());
						}
					}
				} else {
					log.error(" Error : Invalid stock symbol.");
